				  ...
				  INFO "目录$dir存在"
					...
					do
						...
						if [ $global_pre_num -eq 1 ];then
							INFO "$dir中找到一个global-pre.sql文件"
						    echo "--------------global-pre.sql--------------" >> $init_sql_file
							cat global-pre.sql >> $init_sql_file
							INFO "在$element的init.sql中追加global-pre.sql"
						fi
						...
						if [ $global_pre_num -eq 1 ];then
							INFO "$dir中找到一个global-post.sql文件"
							echo "--------------global-post.sql--------------" >> $init_sql_file
							cat global-post.sql >> $init_sql_file
							INFO "在$element的init.sql中追加global-post.sql"
						fi
						# 追加对应版本的sql
						cd $element
						for sql_file in `ls`
						do  
						   # INFO "$element 文件夹中含有：`ls`"
							echo "--------------sql_file--------------" >> $init_sql_file
							cat $sql_file >> $init_sql_file
							INFO "在$element的init.sql中追加$sql_file"
						done
						cd ..
